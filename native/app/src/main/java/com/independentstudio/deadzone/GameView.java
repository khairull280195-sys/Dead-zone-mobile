...
public final class GameView extends GLSurfaceView {
    ...
    static final class World implements GLSurfaceView.Renderer{
        ...
        @Override public void onDrawFrame(GL10 gl){
            ...
            // Abandoned tiled hospital corridor inspired by the supplied reference.
            drawCube(0,-.20f,0,8,.18f,22,.11f,.12f,.12f,1);drawCube(-8,2.35f,0,.18f,2.55f,22,.34f,.42f,.45f,1);drawCube(8,2.35f,0,.18f,2.55f,22,.34f,.42f,.45f,1);drawCube(0,4.88f,0,8,.12f,22,.15f,.16f,.17f,1);drawCube(0,2.35f,-22,8,2.55f,.18f,.20f,.22f,.22f,1);
            // Floor and wall tile seams.
            for(int x=-6;x<=6;x+=2)drawCube(x,-.005f,0,.025f,.012f,22,.025f,.028f,.030f,1);for(int z=-20;z<=20;z+=2){drawCube(0,-.004f,z,8,.013f,.025f,.025f,.028f,.030f,1);drawCube(-7.80f,2.35f,z,.025f,2.50f,.022f,.06f,.08f,.085f,1);drawCube(7.80f,2.35f,z,.025f,2.50f,.022f,.06f,.08f,.085f,1);}
            for(float y=.7f;y<4.8f;y+=.8f){drawCube(-7.79f,y,0,.022f,.022f,22,.055f,.075f,.08f,1);drawCube(7.79f,y,0,.022f,.022f,22,.055f,.075f,.08f,1);}
            // Ceiling panels, fluorescent lights, pipes and damaged doors.
            for(int z=-18;z<=18;z+=6){drawCube(0,4.70f,z,1.25f,.055f,.32f,.72f,.75f,.68f,1);drawCube(-3.8f,4.52f,z,.07f,.07f,2.1f,.10f,.12f,.13f,1);}
            for(int z=-14;z<=14;z+=14){drawCube(-7.55f,2.0f,z,.16f,1.65f,1.35f,.10f,.12f,.13f,1);drawCube(7.55f,2.0f,z+5,.16f,1.65f,1.35f,.10f,.12f,.13f,1);}
            drawCube(-5.5f,.45f,-6,.65f,.45f,.65f,.22f,.15f,.09f,1);drawCube(5.6f,.50f,8,.42f,.50f,.42f,.18f,.10f,.055f,1);
            // Blood trails and a flickering fire at the blocked end of the corridor.
            drawRound(-1.2f,.025f,-3,.80f,.018f,.35f,.23f,.012f,.010f);drawRound(2.1f,.026f,-10,.55f,.018f,.32f,.18f,.010f,.008f);
            for(int i=0;i<9;i++){float flame=.45f+.28f*(float)Math.sin(sceneTime*6+i),px=-2.4f+i*.60f;drawRound(px,.35f+flame*.35f,-20.7f,.28f,.38f+flame,.22f,i%2==0?1:.92f,i%2==0?.18f:.42f,.015f);}
            for(int i=0;i<5;i++)drawRound(-1.8f+i*.9f,2.1f+(i%2)*.45f,-20.3f,.55f,.45f,.35f,.055f,.045f,.042f);
        ...
        private void update(float dt){
            ...
            playerX+=(forwardX*moveForward+sideX*moveSide)*4.1f*dt;playerZ+=(forwardZ*moveForward+sideZ*moveSide)*4.1f*dt;playerX=clamp(playerX,-7.30f,7.30f);playerZ=clamp(playerZ,-20.8f,20.8f);
        ...
        private void spawn(){Zombie z=new Zombie();float a=rng.nextFloat()*6.283f;z.x=clamp(playerX+(float)Math.sin(a)*15,-6.6f,6.6f);z.z=clamp(playerZ+(float)Math.cos(a)*15,-20,20);z.phase=rng.nextFloat()*6.28f;z.variant=rng.nextInt(3);zombies.add(z);}
        private void spawnBoss(){Zombie z=new Zombie();float a=rng.nextFloat()*6.283f;z.x=clamp(playerX+(float)Math.sin(a)*18,-5.8f,5.8f);z.z=clamp(playerZ+(float)Math.cos(a)*18,-19,19);z.phase=rng.nextFloat()*6.28f;z.hp=700+wave*65;z.boss=true;zombies.add(z);sounds.growl();}
        ...
        private void drawZombie(Zombie z){
            ...
            float shirtR=z.variant==0?.48f:z.variant==1?.58f:.16f,shirtG=z.variant==0?.43f:z.variant==1?.59f:.10f,shirtB=z.variant==0?.34f:z.variant==1?.56f:.08f;
            ...
            if(z.variant==0){drawRound(z.x,base+2.40f+bob,z.z+.05f,.33f,.23f,.31f,.025f,.020f,.018f);drawPart(z.x-.28f,base+2.12f+bob,z.z+.05f,.075f,.28f,.07f,0,face,bodyTilt,.025f,.020f,.018f);drawPart(z.x+.28f,base+2.12f+bob,z.z+.05f,.075f,.28f,.07f,0,face,bodyTilt,.025f,.020f,.018f);}else if(z.variant==1){drawPart(z.x,base+.98f+bob,z.z,.44f,.28f,.29f,0,face,bodyTilt,.52f,.54f,.52f);}else{drawRound(z.x,base+2.47f+bob,z.z,.32f,.14f,.30f,.48f,.035f,.025f);}
        ...
        private void drawHud(){
            ...
            for(Zombie z:zombies)if(z.death<0)drawRound(mx+z.x/8f*(r-12),my-z.z/22f*(r-12),.10f,z.boss?10:5.5f,z.boss?10:5.5f,.02f,z.boss?1:.95f,z.boss?.35f:.04f,.02f);
            drawRound(mx+playerX/8f*(r-12),my-playerZ/22f*(r-12),.12f,7,7,.02f,.05f,.85f,.30f);
